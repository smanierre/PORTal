export class SearchableList extends HTMLElement {
  ...
  constructor() {
    ...
    if (addBtn instanceof HTMLButtonElement) {
      ...
      console.error(
        "There should be a button with the class 'searchable-list-add' in this component!",
      );
    ...
    if (clearBtn instanceof HTMLButtonElement) {
      ...
      console.error(
        "There should be a button in the '.searchable-list-search' div!",
      );
  ...
  handleFilter(e: Event) {
    ...
    if (e.target instanceof HTMLInputElement) {
      ...
      this.items.forEach((item) => {
        if (
          !item.textContent?.toLowerCase().includes(input.value.toLowerCase())
        ) {
          item.hidden = true;
        } else {
          item.hidden = false;
