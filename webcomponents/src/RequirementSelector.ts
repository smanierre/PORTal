...
export class RequirementSelector extends HTMLElement {
  ...
  constructor() {
    ...
    if (addButton instanceof HTMLButtonElement) {
      ...
      console.error(
        "There should be a button with the id 'add-button' in this component!",
      );
    ...
    if (requirementList instanceof HTMLSpanElement) {
      ...
      console.error(
        "There should be a span inside an element with class 'requirement-items' in this component!",
      );
            ...
            if (res.ok) {
              res
                ...
                .then((html) => {
                  ...
                  if (this.#itemElement === null) {
                    reject(
                      "expected element wasn't retrieved from the server.",
                    );
                    return;
                  }
            ...
            if (res.ok) {
              res
                ...
                .then((html) => {
                  ...
                  const elem = parser.parseFromString(html, "text/html");
                  this.#editorElement =
                    elem.querySelector("requirement-editor");
                  if (this.#editorElement === null) {
                    reject(
                      "expected element wasn't retrieved from the server.",
                    );
              ...
              reject(
                new Error("Failed to load requirement editor from server!"),
              );
            }
          })
          .catch(reject);
  ...
  connectedCallback() {
    ...
    this.requirements.forEach(async (requirement) => {
      ...
      if (req instanceof HTMLButtonElement) {
        ...
        const cancelIcon = req.querySelector(".remove-requirement-button");
  ...
  async newRequirement() {
    ...
    if (editor instanceof HTMLElement) {
      editor.setAttribute(
        "data-parent-qualification-id",
        this.dataset.parentQualificationId || "",
      );
    }
  ...
  async updateRequirement(e: Event) {
    ...
    this.dialog.innerHTML = "";
    this.dialog.appendChild(editor);
    const req = this.requirements.filter(
      (req) => req.id === target?.dataset.id,
    );
    editor.setFormData(req[0]);
    this.dialog.showModal();
  }

  async setAddedOrUpdatedQualificationRequirement(
