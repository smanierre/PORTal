...
export class QualificationEditor extends HTMLElement {
    ...
    constructor() {
        ...
        if (submitBtn instanceof HTMLButtonElement) {
            ...
            console.error("This component should have a button with class 'form-submit'!")
        ...
        const initialRequirementsSelector = this.querySelector('requirement-selector[data-reqtype="initial_requirements"]')
        if (initialRequirementsSelector !== null) {
            this.initialRequirementsSelector = initialRequirementsSelector as RequirementSelector;
        } else {
            console.error("This component should have a requirement-selector element with data-reqtype='initial_requirements'");
            this.initialRequirementsSelector = new RequirementSelector();
        }

        const recurringRequirementsSelector = this.querySelector('requirement-selector[data-reqtype="recurring_requirements"]')
        if (recurringRequirementsSelector !== null) {
            this.recurringRequirementsSelector = recurringRequirementsSelector as RequirementSelector
        } else {
            console.error("This component should have a requirement-selector element with data-reqtype='recurring_requirements'")
        ...
        if (expirationInput instanceof HTMLInputElement) {
            ...
            console.error("This component should have an input element with name 'expires'!")
            this.expirationInput = new HTMLInputElement
        }

        const expirationIntervalInput = this.querySelector('input[name="expiration_interval"]')
        if (expirationIntervalInput?.parentElement instanceof HTMLLabelElement) {
            ...
            console.error("This component should have an input element with name 'expiration_interval' with a parent label!")
        ...
        if (expirationIntervalInput instanceof HTMLInputElement) {
            ...
            console.error("This component should have an input element with name 'expiration_interval'!")
            this.expirationIntervalInput = new HTMLInputElement
        }

        const initialRequirementsInput = this.querySelector('input[name="initial_requirements"]')
        if (initialRequirementsInput instanceof HTMLInputElement) {
            this.initialRequirementsInput = initialRequirementsInput
        } else {
            console.error("This component should have an input element with name 'initial_requirements'!")
            this.initialRequirementsInput = new HTMLInputElement
        }

        const recurringRequirementsInput = this.querySelector('input[name="recurring_requirements"]')
        if (recurringRequirementsInput instanceof HTMLInputElement) {
            this.recurringRequirementsInput = recurringRequirementsInput
        } else {
            console.error("This component should have an input element with name 'recurring_requirements'!")
    ...
    get recurringRequirements(): Requirement[] {
        const data = this.recurringRequirementsSelector.getAttribute("requirements")
        return data === null ? [] : JSON.parse(data)
    }
    ...
    connectedCallback() {
        ...
        this.expirationInput.addEventListener("change", this.handleExpirationChange.bind(this));
        this.form.addEventListener("change", this.handleFormUpdate.bind(this));
        this.initialRequirementsSelector.addEventListener("updated", this.handleFormUpdate.bind(this));
    }

    disconnectedCallback() {
        this.submitBtn.removeEventListener("click", this.handleSubmit.bind(this));
        this.expirationInput.removeEventListener("change", this.handleExpirationChange.bind(this))
        this.form.addEventListener("change", this.handleFormUpdate.bind(this));
        this.initialRequirementsSelector.removeEventListener("updated", this.handleFormUpdate.bind(this));
    }
    ...
    handleFormUpdate() {
        const valid = validateQualificationEditor(new FormData(this.form), this.initialRequirements, this.recurringRequirements);
        if (valid) {
    ...
    handleSubmit(e: Event) {
        ...
        const cleanInitialReqs = this.initialRequirements.map(requirement => {
            if (requirement.id.length < 20) { // It would take a lottttttttt of temporary ID's for the dummy IDs to be as long as a UUID
                requirement.id = ""
            }
            return requirement
        })
