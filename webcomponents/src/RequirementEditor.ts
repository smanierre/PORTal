...
export class RequirementEditor extends HTMLElement {
    ...
    constructor() {
        ...
        this.setNonQualificationDisplay = this.setNonQualificationDisplay.bind(this);
        ...
        if (this.parentElement?.parentElement instanceof RequirementSelector) {
            ...
            console.error("This elements 2x parent needs to be a requirement selector!")
        ...
        const qualificationInput = this.querySelector("select[name=qualification_id]")
        ...
        const qualificationLabel = this.querySelector("label[for=qualification_id]")
    ...
    connectedCallback() {
        ...
        const qualificationOptions = this.qualificationInput?.querySelectorAll("option")
    ...
    async handleSubmit(e: Event) {
        e.preventDefault();
        if (this.typeInput?.value !== "WBT" && this.typeInput?.value !== "Qualification" && this.typeInput?.value !== "Qualification") {
            return
        }
        const req: Requirement = {
            id: this.idInput?.value || "",
            days_valid_for: this.daysValidForInput?.value ? +this.daysValidForInput.value : 0,
        ...
        if (req.type === "Qualification" && req.qualification_id) {
            this.requirementSelector.setAddedOrUpdatedQualificationRequirement(req, this.qualificationInput![this.qualificationInput!.selectedIndex].textContent!)
            this.dialog?.close()
            return
        }
        ...
        if (e.target instanceof HTMLSelectElement) {
            switch (e.target.value) {
                ...
                case "Grade":
                    this.setNonQualificationDisplay();
                    break;
                case "Qualification": this.nameLabel!.hidden = true;
                    this.nameInput!.disabled = true;
