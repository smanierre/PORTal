...
const (
	addRequirementQuery                  = "INSERT INTO requirement(id, name, notes, grade, days_valid_for, qualification_id, reference, type) VALUES(?, ?, ?, ?, ?, ?, ?, ?);"
	...
	updateRequirementQuery               = "UPDATE requirement SET name=?, notes=?, grade=?, days_valid_for=?, qualification_id= ?, reference=?, type=? WHERE id=?;"
